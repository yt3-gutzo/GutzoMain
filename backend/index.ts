...
console.log(
  "Gutzo Marketplace Server starting...:",
  Deno.env.get("PRIVATE_SUPABASE_URL"),
);
// Do not log secrets in production
if (Deno.env.get("NODE_ENV") === "development") {
  console.log(
    "Gutzo Marketplace Server starting... (service role key is configured)",
  );
}
...
async function checkDatabaseSchema() {
  try {
    ...
    if (usersError?.message?.includes('relation "users" does not exist')) {
      ...
      console.log(
        "💡 Run the migration: /supabase/migrations/create_users_table.sql",
      );
    } else if (usersError) {
    ...
    if (
      error?.message?.includes(
        'relation "product_subscriptions" does not exist',
      )
    ) {
      console.log("⚠️  product_subscriptions table does not exist");
      console.log(
        "💡 Run the migration: /supabase/migrations/create_product_subscriptions_table.sql",
      );
    } else if (error) {
      console.error(
        "❌ Error checking product_subscriptions table:",
        error.message,
      );
    } else {
...
app.post("/gutzo-api/save-order", async (c) => {
  try {
    ...
    if (
      !orderId || !userPhone || !items || !Array.isArray(items) ||
      items.length === 0 || !totalAmount || !vendorId
    ) {
      return c.json(
    ...
    const { data: user, error: userError } = await supabase.from("users")
      .select("id").eq("phone", userPhone).single();
    if (userError || !user) {
      return c.json({
        success: false,
    ...
    const { data: orderData, error: orderError } = await supabase.from("orders")
      .insert(orderInsert).select().single();
...
app.post("/gutzo-api/create-phonepe-payment", async (c) => {
  ...
  if (redirectUrl.includes("localhost") || redirectUrl.includes("127.0.0.1")) {
    console.warn(
      "⚠️ PhonePe redirectUrl is localhost. Use a public URL for UAT/production.",
    );
  }
  let token;
  try {
    ...
    return c.json({
      success: false,
      error: "Failed to get PhonePe token",
      details: e.message,
    }, 500);
  }
  const payload = {
  ...
  try {
    ...
    return c.json({
      success: false,
      error: "Error calling PhonePe API",
...
app.post("/gutzo-api/phonepe-webhook", async (c) => {
  try {
    ...
    const headerVerify = c.req.header("x-verify") || c.req.header("X-VERIFY") ||
      "";
    const headerMerchant = c.req.header("x-merchant-id") ||
    ...
    const sigExpected = PHONEPE_SALT_KEY
      ? (await sha256Hex(rawBody + PHONEPE_SALT_KEY)) + "###" +
        (PHONEPE_SALT_INDEX || "")
      : "";
    ...
    const sigExpectedAlt = PHONEPE_SALT_KEY
      ? (await sha256Hex(base64Body + PHONEPE_SALT_KEY)) + "###" +
        (PHONEPE_SALT_INDEX || "")
      : "";

    const verified = headerVerify === sigExpected ||
      headerVerify === sigExpectedAlt;
    if (!verified) {
    ...
    function mapPaymentStatus(codeVal: string, stateVal: string) {
      ...
      if (up.includes("SUCCESS") || st === "COMPLETED" || st === "SUCCESS") {
        return { payment_status: "paid", order_status: "confirmed" };
      }
      if (up.includes("PENDING") || st === "PENDING") {
    ...
    if (merchantTransactionId) {
      const { data: updated, error } = await supabase
        ...
        .update({
          ...
          payment_method: "phonepe",
          payment_id: transactionId || body?.data?.providerReferenceId ||
            body?.providerReferenceId || null,
      ...
      if (error) {
        ...
        console.log(
          `[PhonePe Webhook] Updated orders for ${merchantTransactionId}. Rows: ${
            updated?.length || 0
          }`,
        );
      }
      ...
      console.warn(
        "[PhonePe Webhook] Missing merchantTransactionId in webhook payload",
      );
    }
...
app.get("/gutzo-api/phonepe-status/:orderId", async (c) => {
  ...
  try {
    ...
    return c.json({
      success: false,
      error: "Error calling PhonePe status API",
      details: err?.message || err,
    }, 500);
  }
});
...
app.post("/gutzo-api/phonepe-refund", async (c) => {
  ...
  try {
    ...
    return c.json({
      success: false,
      error: "Error calling PhonePe refund API",
      details: err?.message || err,
    }, 500);
  }
});
...
app.get("/gutzo-api/health", async (c) => {
  try {
    ...
    const { data, error } = await supabase.from("vendors").select("id").limit(
      1,
    );
    if (error) {
    ...
    const { data: subscriptionTest, error: subscriptionError } = await supabase
      .from("product_subscriptions").select("id").limit(1);
    const subscriptionStatus = subscriptionError?.message?.includes(
        'relation "product_subscriptions" does not exist',
      )
...
app.get("/gutzo-api/vendors", async (c) => {
  try {
    ...
    console.log(
      `Successfully fetched ${data?.length || 0} vendors from database`,
    );
  ...
  try {
    ...
    console.log(
      `Successfully fetched ${
        data?.length || 0
      } products for vendor ${vendorId} from database`,
    );
    ...
    const availableCount = productsWithAvailability.filter((p) =>
      p.available
    ).length;
    const unavailableCount = productsWithAvailability.filter((p) =>
      !p.available
    ).length;
...
app.get("/gutzo-api/products", async (c) => {
  try {
    ...
    console.log(
      `Successfully fetched ${data?.length || 0} products from all vendors`,
    );
    ...
    const availableCount = allProductsWithAvailability.filter((p) =>
      p.available
    ).length;
    const unavailableCount = allProductsWithAvailability.filter((p) =>
      !p.available
    ).length;
...
app.get("/gutzo-api/categories", async (c) => {
  try {
    ...
    console.log(
      `Successfully fetched ${data?.length || 0} categories from database`,
    );
    return c.json(data || []);
  } catch (error) {
...
app.get("/gutzo-api/products/available", async (c) => {
  try {
    ...
    console.log(
      `📦 Available Products Summary: ${availableProducts.length} products`,
    );
...
app.get("/gutzo-api/products/:id/subscription", async (c) => {
  try {
    ...
    const { data, error } = await supabase.from("product_subscriptions").select(
      "id, product_id, has_subscription",
...
app.post("/gutzo-api/products/:id/subscription", async (c) => {
  try {
    ...
    if (existingData) {
      ...
      const { data, error } = await supabase.from("product_subscriptions")
        .update({
...
app.get("/gutzo-api/subscriptions", async (c) => {
  try {
    ...
    console.log(
      `Successfully fetched ${
        data?.length || 0
      } product subscriptions from database`,
    );
    return c.json(data || []);
  } catch (error) {
...
app.post("/gutzo-api/vendors", async (c) => {
  try {
    ...
    const { data, error } = await supabase.from("vendors").insert(vendorData)
      .select().single();
    if (error) {
...
app.post("/gutzo-api/notifications", async (c) => {
  try {
    ...
    const { data: existingWaitlist, error: waitlistCheckError } = await supabase
      .from("waitlist").select("id").eq("email", email.toLowerCase()).single();
    ...
    const { data: waitlistData, error: waitlistError } = await supabase.from(
      "waitlist",
    ).insert({
...
app.post("/gutzo-api/check-user", async (c) => {
  try {
    ...
    const { data: userData, error: userError } = await supabase.from("users")
      .select("id, phone, name, email, created_at").eq("phone", phone).single();
    if (userError && userError.code !== "PGRST116") {
      console.error("Database error checking user:", userError);
      // Handle specific database schema issues
      if (
        userError.message?.includes("column users.phone does not exist") ||
        userError.message?.includes('relation "users" does not exist')
      ) {
        return c.json({
          success: false,
          error: "Database schema issue detected",
          details:
            "Users table or phone column missing. Please run database migrations.",
          schema_error: true,
    ...
    console.log(
      `User ${exists ? "exists" : "does not exist"} for phone: ${phone}`,
    );
    return c.json({
      success: true,
      exists,
...
app.post("/gutzo-api/create-user", async (c) => {
  try {
    ...
    const { data: existingUser, error: checkError } = await supabase.from(
      "users",
    ).select("id").eq("phone", phone).single();
    if (checkError && checkError.code !== "PGRST116") {
      // Handle schema errors specifically
      if (
        checkError.message?.includes("column users.phone does not exist") ||
...
app.post("/gutzo-api/verify-otp", async (c) => {
  try {
    ...
    // Validate inputs
    if (
      !phone || !otp || typeof phone !== "string" || typeof otp !== "string"
    ) {
      return c.json({
        success: false,
    ...
    const { data: otpRecord, error: queryError } = await supabase.from(
      "otp_verification",
    ).select("*").eq("phone", phone).eq("otp", otp).eq("verified", false).order(
      "created_at",
      {
        ascending: false,
      },
    ...
    const { error: updateError } = await supabase.from("otp_verification")
      .update({
        verified: true,
...
app.post("/gutzo-api/validate-user", async (c) => {
  try {
    ...
    console.log(
      `✅ User validation result: phone=${phone}, exists=${userExists}, verified=${verified}`,
    );
    return c.json({
      userExists,
      verified,
...
app.post("/gutzo-api/get-user", async (c) => {
  try {
    ...
    if (
      !normalizedPhone.startsWith("+91") && !normalizedPhone.startsWith("91")
    ) {
    ...
    console.log(
      "📱 Normalized phone number for database query:",
      normalizedPhone,
    );
    // Query users table to get user information using normalized phone
    const { data: userData, error: userError } = await supabase.from("users")
      .select("id, name, phone, email, verified, created_at").eq(
        "phone",
        normalizedPhone,
      ).single();
    if (userError?.code === "PGRST116") {
      ...
      console.log(
        "❌ User not found with normalized phone, trying alternative formats...",
      );
      // Try without +91 prefix
      const alternativePhone = phone.toString().trim();
      const { data: altUserData, error: altUserError } = await supabase.from(
        "users",
      ).select("id, name, phone, email, verified, created_at").eq(
        "phone",
        alternativePhone,
      ...
      if (altUserError?.code === "PGRST116") {
        console.log(
          "❌ User not found in database for phone:",
          phone,
          "or",
          normalizedPhone,
        );
        ...
        console.error(
          "❌ Database error fetching user with alternative phone:",
          altUserError,
        );
        return c.json({
        ...
        console.log(
          "✅ User found with alternative phone format:",
          alternativePhone,
        );
        return c.json({
          id: altUserData.id,
...
app.post("/gutzo-api/get-user-cart", async (c) => {
  try {
    ...
    const { data: cartData, error: cartError } = await supabase.from("cart")
      .select("product_id, vendor_id, quantity, created_at").eq(
        "user_phone",
        userPhone,
      );
    ...
    console.log(
      `✅ Found ${cartData.length} cart items, fetching fresh product data...`,
    );
    ...
    const { data: productsData, error: productsError } = await supabase.from(
      ...
      `).in("id", productIds);
    console.log("get-user-cart: Products query result:", {
      productsData,
      productsError,
    });
    if (productsError) {
    ...
    const enrichedCartItems = cartData.map((cartItem) => {
      ...
      if (!product) {
        console.warn(
          `⚠️ Product ${cartItem.product_id} not found, skipping cart item`,
        );
        return null;
      }
      return {
    ...
    console.log(
      `✅ Returning enriched cart: ${enrichedCartItems.length} items, total: ₹${totalAmount}`,
    );
    return c.json({
...
app.post("/gutzo-api/save-user-cart", async (c) => {
  try {
    ...
    console.log(
      `💾 Saving cart for user ${userPhone} with ${items.length} items`,
    );
    // Clear existing cart items for user
    const { error: deleteError } = await supabase.from("cart").delete().eq(
      "user_phone",
      userPhone,
    );
    if (deleteError) {
    ...
    console.log(
      `✅ Successfully saved ${items.length} cart items for user ${userPhone}`,
    );
    return c.json({
      success: true,
...
app.post("/gutzo-api/update-cart-item", async (c) => {
  try {
    ...
    if (quantity === 0) {
      ...
      console.log(
        `🗑️ Removed item ${productId} from cart for user ${userPhone}`,
      );
    } else {
      ...
      console.log(
        `✅ Updated cart item ${productId} to quantity ${quantity} for user ${userPhone}`,
      );
    }
    return c.json({
      success: true,
...
async function getUserIdFromPhone(phone) {
  ...
  const { data: user, error } = await supabase.from("users").select("id").eq(
    "phone",
    phone,
  ).single();
  if (error) {
...
app.get("/gutzo-api/user-addresses/:phone", async (c) => {
  try {
    ...
    const { data, error } = await supabase.from("user_addresses").select("*")
      .eq("user_id", userId).order("is_default", { ascending: false }).order(
        "created_at",
        { ascending: false },
      );
    if (error) {
...
app.get("/gutzo-api/user-addresses/:phone/available-types", async (c) => {
  try {
    ...
    const { data, error } = await supabase.from("user_addresses").select("type")
      .eq("user_id", userId);
    if (error) {
      return c.json({
...
app.post("/gutzo-api/user-addresses", async (c) => {
  try {
    ...
    if (isDefault) {
      await supabase.from("user_addresses").update({ is_default: false }).eq(
        "user_id",
        userId,
      );
    }
...
app.delete("/gutzo-api/user-addresses/:id", async (c) => {
  try {
    ...
    const { data: existing, error: fetchError } = await supabase.from(
      "user_addresses",
    ).select("id").eq("id", addressId).single();
    if (fetchError) {
    ...
    const { error: deleteError } = await supabase.from("user_addresses")
      .delete().eq("id", addressId);
    if (deleteError) {
...
app.post("/gutzo-api/paytmint", async (c) => {
  try {
    ...
    // @ts-ignore
    if (
      typeof globalThis.Deno !== "undefined" && globalThis.Deno.env &&
    ...
    if (!resp.ok) {
      return c.json({
        success: false,
        error: "Initiate transaction service error",
        details: result,
      }, 500);
    }

    const txnToken = result.initiateTransactionResponse?.body?.txnToken;

    return c.json({
      success: true,
      txnToken, // Frontend looks for data.txnToken
      mid: mid, // Return mid explicitly
      paytmResponse: result.initiateTransactionResponse, // Frontend looks for data.paytmResponse
